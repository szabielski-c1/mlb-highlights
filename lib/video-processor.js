...
export async function createRundownVideo(clipsWithSelections, gamePk, titleCardUrl = null) {
  ...
  // Build a list that marks which clips are transitions (need hard cuts) vs play clips (need dissolves)
  // We need to track this because transition clips should cut, not dissolve
  const clipInfos = [];
  let pathIndex = 0;

  // If title card was added, it's first
  if (titleCardUrl && trimmedPaths.length > 0) {
    clipInfos.push({ path: trimmedPaths[pathIndex++], isTransition: false });
  }

  // Map original clips to their processed paths
  for (const clip of clipsWithSelections) {
    if (clip.isTransition && clip.transitionPath) {
      // This was a transition - find its path
      const idx = trimmedPaths.indexOf(clip.transitionPath);
      if (idx >= 0) {
      ...
      if (pathIndex < trimmedPaths.length) {
        clipInfos.push({ path: trimmedPaths[pathIndex++], isTransition: false });
      }
    }
  }
