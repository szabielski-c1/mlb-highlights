...
export async function concatenateVideos(videoPaths, outputPath, transitionFrames = 10) {
  ...
  // For dissolve transitions, we need to use ffmpeg's xfade filter
  // Build complex filter graph for video and audio crossfades
  const inputs = videoPaths.map((p, i) => `-i "${p}"`).join(' ');

  // Build the filter chain for xfade (video) and acrossfade (audio)
  // Each transition overlaps by transitionDuration seconds
  let videoFilter = '';
  let audioFilter = '';

  if (videoPaths.length === 2) {
    // Simple case: just two videos
    videoFilter = `[0:v][1:v]xfade=transition=fade:duration=${transitionDuration}:offset=OFFSET0[outv]`;
    audioFilter = `[0:a][1:a]acrossfade=d=${transitionDuration}[outa]`;
  } else {
    // Multiple videos: chain xfades together
    // First transition
    videoFilter = `[0:v][1:v]xfade=transition=fade:duration=${transitionDuration}:offset=OFFSET0[v1]`;
    audioFilter = `[0:a][1:a]acrossfade=d=${transitionDuration}[a1]`;
    ...
    for (let i = 2; i < videoPaths.length; i++) {
      ...
      if (i === videoPaths.length - 1) {
        // Last transition - output to final
        videoFilter += `;[v${prevLabel}][${i}:v]xfade=transition=fade:duration=${transitionDuration}:offset=OFFSET${prevLabel}[outv]`;
        audioFilter += `;[a${prevLabel}][${i}:a]acrossfade=d=${transitionDuration}[outa]`;
      } else {
        videoFilter += `;[v${prevLabel}][${i}:v]xfade=transition=fade:duration=${transitionDuration}:offset=OFFSET${prevLabel}[v${nextLabel}]`;
        audioFilter += `;[a${prevLabel}][${i}:a]acrossfade=d=${transitionDuration}[a${nextLabel}]`;
      }
    }
  }

  // We need to get durations of each video to calculate offsets
  const durations = [];
  ...
  // Calculate offsets: each offset is cumulative duration minus cumulative transitions
  let cumulativeDuration = durations[0];
  for (let i = 0; i < videoPaths.length - 1; i++) {
    const offset = cumulativeDuration - transitionDuration;
    videoFilter = videoFilter.replace(`OFFSET${i}`, offset.toFixed(3));
