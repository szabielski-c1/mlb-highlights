...
/**
 * Simple concatenation using concat filter - normalizes all inputs
 * No transitions, just joins clips end to end with hard cuts
...
async function concatenateVideosSimple(videoPaths, outputPath) {
  ...
  // This normalizes framerate and timebase across all inputs
  const inputs = [];
  const filterParts = [];

  for (let i = 0; i < videoPaths.length; i++) {
    inputs.push('-i', videoPaths[i]);
    // Normalize each input to 30fps with consistent timebase
    filterParts.push(`[${i}:v]fps=30,settb=AVTB,setpts=PTS-STARTPTS[v${i}]`);
  ...
  filterParts.push(`${concatInputs}concat=n=${videoPaths.length}:v=1:a=1[outv][outa]`);
...
/**
 * Concatenate videos with transition-aware logic:
 * - Use hard cuts (no transition) before and after inning transition clips
 * - Use dissolve transitions between regular play clips
 *
 * @param {Array<{path: string, isTransition: boolean}>} clipInfos - Clips with transition flags
 * @param {string} outputPath - Output file path
 * @returns {Promise<string>} - Path to concatenated video
 */
async function concatenateVideosWithTransitionAwareness(clipInfos, outputPath) {
  if (clipInfos.length === 0) {
    throw new Error('No clips to concatenate');
  }

  if (clipInfos.length === 1) {
    await fs.copyFile(clipInfos[0].path, outputPath);
    return outputPath;
  }

  // Strategy: Group consecutive non-transition clips together, apply dissolves within groups,
  // then hard-cut between groups (separated by transitions)
  //
  // Example: [play1, play2, TRANS, play3, play4, TRANS, play5]
  // -> dissolve(play1, play2) -> cut -> TRANS -> cut -> dissolve(play3, play4) -> cut -> TRANS -> cut -> play5

  const tempDir = path.dirname(outputPath);
  const segments = []; // Each segment is either a single transition or a group of plays with dissolves

  let currentGroup = [];

  for (const clip of clipInfos) {
    if (clip.isTransition) {
      // Flush current group as a dissolved segment
      if (currentGroup.length > 0) {
        segments.push({ type: 'plays', clips: currentGroup });
        currentGroup = [];
      }
      // Add transition as its own segment
      segments.push({ type: 'transition', path: clip.path });
    } else {
      currentGroup.push(clip.path);
    }
  }
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (segment.type === 'transition') {
      // Transition clips are used directly
      segmentPaths.push(segment.path);
    } else if (segment.type === 'plays') {
      if (segment.clips.length === 1) {
        // Single clip, use directly
        segmentPaths.push(segment.clips[0]);
      } else {
        // Multiple clips - apply dissolve transitions within this group
        const groupOutputPath = path.join(tempDir, `group-${i}.mp4`);
        await concatenateVideos(segment.clips, groupOutputPath);
        segmentPaths.push(groupOutputPath);
      }
    }
  }

  // Finally, hard-cut all segments together
  ...
  // Cleanup intermediate group files (but not source files)
  for (let i = 0; i < segments.length; i++) {
    if (segments[i].type === 'plays' && segments[i].clips.length > 1) {
      const groupPath = path.join(tempDir, `group-${i}.mp4`);
...
export async function createRundownVideo(clipsWithSelections, gamePk, titleCardUrl = null) {
  ...
  // Track clips with their transition status as we build the list
  const clipInfos = [];
  const filesToCleanup = [];

  // If we have a title card URL, extract first 1.5 seconds
  if (titleCardUrl) {
    try {
      ...
      clipInfos.push({ path: titleCardPath, isTransition: false });
  ...
  for (let i = 0; i < clipsWithSelections.length; i++) {
    ...
    if (clip.isTransition && clip.transitionPath) {
      try {
        ...
        // Add the transition - mark it as a transition for hard cuts
        clipInfos.push({ path: clip.transitionPath, isTransition: true });
    ...
    try {
      ...
      clipInfos.push({ path: trimmedPath, isTransition: false });
  ...
  console.log(`Processing ${clipInfos.length} clips:`, clipInfos.map(c =>
    `${path.basename(c.path)}${c.isTransition ? ' (transition)' : ''}`
  ).join(', '));

  // Concatenate with smart transitions:
  // - Hard cuts before/after inning transitions
  // - Dissolves between regular play clips
  const finalPath = path.join(tempDir, `rundown-${gamePk}.mp4`);
  await concatenateVideosWithTransitionAwareness(clipInfos, finalPath);
